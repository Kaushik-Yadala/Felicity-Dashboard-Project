    ...
    useEffect(() => {
        const loadDashboard = async () => {
            try {
                setLoading(true);
                setError(null);
                const response = await apiClient.get("/organizer/event/ongoing");
                // console.log("Dashboard response:", response);
                if (response?.data?.events) {
                    setEvents(response.data.events);
                    console.log("Dashboard events:", response.data.events);
                }
            } catch (error) {
                console.error("Error loading ongoing events:", error);
                setError("Failed to load ongoing events. Please try again.");
            } finally {
                setLoading(false);
            }
        }
    ...
    return (
        ...
        <div className="p-6 max-w-8xl mx-auto space-y-6 ">
            ...
            <div className="flex flex-wrap justify-start justify-between gap-4 mt-4 relative border-y-6 border-x-6 border-foreground dark:border-ring p-6">
                {loading && <p className="text-muted-foreground w-full">Loading ongoing events...</p>}
